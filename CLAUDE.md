...
- **"Възстанови" от кошчето — `restoreFromTrash()`.** Дотогава единственият
  ...
  **"Изтрий" в кошчето вече казва "Изтрий завинаги"** (`ctxDeleteForever`)
  ...
  `sanitizeImportedNote()` пази `prevTag`/`restoreParentId` при внасяне на
  архив — без това възстановен от бекъп файл потребител би загубил
  възможността изтритите му бележки да се върнат на правилното място.
  Тествано с Playwright, 20 проверки: пълен кръг трийсене → изтичане на
  Undo → "Възстанови" за бележка (с подбележка, с оригинален таг) и за
  самостоятелна подбележка (връща се обратно В родителя, не като нова
  ...
  export→import пази двете нови полета.
