...
- **Напомняния — реално задействане** (`checkRemindersNow()`,
  ...
  **НЕПРОВЕРЕНО НА УСТРОЙСТВО:** нативната част е писана без възможност за
  ...
  нарочно.
- **Обратна връзка ВЪТРЕ в приложението** (`renderFeedbackForm()` /
  `renderFeedbackInbox()`, колекция `feedback`) — бутонът в профила →
  "Помощ" вече отваря поле за писане, НЕ пуска `mailto`. По изрична заявка:
  вместо да поддържаме пощенска кутия, съобщенията отиват във Firestore.
  Профилът се вижда само след вход, значи авторът е известен — `uid`,
  имейлът, езикът и датата се записват автоматично и потребителят не пише
  кой е. Админският акаунт ги чете от "Получени съобщения" (до
  "Статистика", видимо само за него).
  **Зависи от `firestore.rules`** — правилото за `feedback` дава на всеки
  влязъл само `create` (с проверка `uid == request.auth.uid`, за да не се
  праща от чуждо име, и лимит 2000 знака), четене само за админския имейл,
  без `update`/`delete`. **Докато правилата не са качени
  (`firebase deploy --only firestore:rules`), изпращането се отхвърля и
  формата показва грешка.**
  **ОТВОРЕНО:** няма известяване за нови съобщения — трябва да се отвори
  приложението. Обмисляно беше известие по имейл през Resend (Cloud
  Function), но иска отделен deploy и не е правено.
- **Имейл адреси — две РАЗЛИЧНИ роли, не ги сливай.** `SATORI_EMAIL` е
  ...
  "Обратна връзка". Сега съвпадат, но се държат като отделни константи
  съзнателно — да рекламираш админския адрес публично значи да покажеш кой
  акаунт си струва да бъде атакуван (фишинг/опити за влизане).
  **ОТВОРЕНО:** прави се истинска пощенска кутия `support@iforget.eu` в
  SuperHosting (пощата е включена в хостинг плана). Щом заработи,
  `SUPPORT_EMAIL` се сменя — един ред. Дотогава остава личният адрес,
  защото кутия, която не получава поща, е по-лошо от лична.
  **Внимание при настройката:** сайтът сочи към GitHub Pages, а
  `noreply@iforget.eu` изпраща през Resend. Създаването на кутия не ги
  засяга, но ръчна промяна на MX записите може да счупи изпращането на
  писмата за парола/потвърждение.
