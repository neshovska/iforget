...
- **Меню на профила — групирано на 4 подменюта (drill-down), не плосък
  списък от ~13 реда.** `renderProfileMenu()` показва само имейл + цветове
  + 4 групови бутона (Изглед/Данни/Акаунт/Помощ), всеки с иконка +
  стрелка `ICONS.chevronRight` вдясно (`.action-chev`). Клик отваря
  съответната `renderProfileGroup*()` функция (`renderProfileGroupView()`/
  `renderProfileGroupData()`/`renderProfileGroupAccount()`/
  `renderProfileGroupHelp()`), която подменя `#profilePanel` innerHTML НА
  МЯСТО със "Назад" бутон отгоре (`.group-back`, `${ICONS.arrowLeft}`) —
  същият установен принцип като `renderCtxMenuReminder()`/
  `renderCtxMenuColor()` в ctx-менюто. Разпределение: **Изглед** =
  прозрачни бележки + лява ръка; **Данни** = Твоят преглед/Календар/
  Експорт/Импорт/Статистика (админ); **Акаунт** = смяна на парола/имейл +
  Изход/Изтриване на профила (danger бутоните, преди отделни в дъното на
  плоското меню, сега вътре в тази група); **Помощ** = Обратна връзка +
  Поверителност/Общи условия. Всички под-форми/изгледи, достъпни ОТ дадена
  група (напр. `renderPasswordForm()`/`renderEmailForm()`/
  `renderDeleteAccountForm()` от Акаунт, `renderMyReview()`/
  `renderCalendarView()`/`renderAdminStats()` от Данни), при "Отказ"/
  "Назад" се връщат в СЪОТВЕТНАТА група (`renderProfileGroupAccount()`/
  `renderProfileGroupData()`), НЕ направо в горното меню — по-плитка
  навигация. Линковете "Поверителност"/"Общи условия" в Помощ са по ситен
  шрифт (`.small-link`, `font-size:11px`) по изрична молба — по-малко
  визуално тегло от истинските действия. Вече покритият
  `#profilePanel` `mousedown`/`stopPropagation()` фикс (виж по-долу) важи
  автоматично и за новите групови бутони — не е нужен нов код за това.
