...
- **Обратна връзка = директен `mailto:` към `SUPPORT_EMAIL`.** Имаше кратък
  ...
  плюс самият адрес е показан и като видим, избираем текст ПОД бутона (не
  само вътре в линка) — точно защото "бутонът за обратна връзка не работи"
  се оказа реален оплак: `mailto:` не отваря пощенски клиент на машина без
  такъв конфигуриран (чест случай на десктоп браузър), а видимият текст
  остава четим/копируем дори тогава.
...
- **Имейл адреси — две РАЗЛИЧНИ роли, не ги сливай.** `SATORI_EMAIL` е
  ...
  история за DNS настройката). **Не е потвърдено в този разговор дали самата
  пощенска кутия (акаунт/парола в Zoho) е реално създадена и тествана да
  ПОЛУЧАВА поща** — DNS сочи правилно към Zoho, но приемането зависи и от
  наличието на активна кутия там.
...
- **Обратна връзка** — бутон в профила, `mailto:` към `SUPPORT_EMAIL`
  (`info@iforget.eu`, Zoho Mail), плюс самият адрес показан и като видим
  текст под бутона (`mailto:` не отваря клиент навсякъде — виж отделния
  раздел по-горе за пълния контекст и историята с вътрешната форма).
