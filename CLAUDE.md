...
- **Тагове/категории** — вградени "Лични"/"Работни" + произволни
  ...
  бележки независимо от тага им) → Работни → Лични → Satori (ако е
  приложимо). След това потребителските тагове — тези МОГАТ да се
  разместват с дълго натискане (~480ms) + провлачване
  (`attachCatRowDrag()`), редът се пази в `tagOrder` (localStorage +
  Firestore `users/{uid}.tagOrder`, синхронизиран между устройства, виж
  `saveTagOrder()`/`orderedCustomTags()`). Накрая "Изтрити", ако кошчето не
  е празно.
...
- **Long-press меню** (~480ms задържане без движение) — Редактирай /
  Добави подбележка / Сподели / Изтрий / Напомняне. Постоянните бутони
  молив/кошче са премахнати от картите — редакция е през ДВОЕН тап/клик
  върху текста (умишлено не единичен, за да не се влиза случайно в
  редакция докато четеш/скролваш), swipe, или това меню.
...
- **Календарен изглед** ("Календар" в профила, `renderCalendarView()`) —
  седмица (понеделник–неделя) или цял месец, превключваеми с табове, с
  ...
  напомняния (виж легендата под изгледа).
